...
let philosophers: PhilosopherType[] = ["Zeno", "Yoda", "Groot", "Marcus Aurelius", "Johan Cruijff"];

export default function DropDown({philosopher, setPhilosopher}: DropDownProps) {
    return (
                ...
                >
                    <div className="">
                        {philosophers.map((philosopherItem) => (
                            <Menu.Item key={philosopherItem}>
                                {({active}) => (
                                    <button
                                        onClick={() => setPhilosopher(philosopherItem)}
                                        className={classNames(
                                            active ? "bg-gray-100 text-gray-900" : "text-gray-700",
                                            philosopher === philosopherItem ? "bg-gray-200" : "",
                                            "px-4 py-2 text-sm w-full text-left flex items-center space-x-2 justify-between"
                                        )}
                                    >
                                        <span>{philosopherItem}</span>
                                        {philosopher === philosopherItem ? (
